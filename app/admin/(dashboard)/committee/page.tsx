...
import { PUBLIC_COMMITTEE_YEAR_LIMIT } from "@/lib/committee";
...
export default async function AdminCommitteePage() {
  ...
  // Rows are already sorted newest first, so anything past the limit is archived.
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Manage committee years and their members. The {PUBLIC_COMMITTEE_YEAR_LIMIT} most recent years are shown
            publicly; older ones stay here for your records but are hidden from the site.
          </p>
        </div>
          ...
          <div key={y.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-card p-4">
            <div>
              <div className="flex items-center gap-2 font-medium text-foreground">
                ...
                {i >= PUBLIC_COMMITTEE_YEAR_LIMIT && <Badge variant="warning">Archived — admin only</Badge>}
              </div>
